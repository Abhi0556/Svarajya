...
import { IncomeStore, IncomeType, Frequency, RiskLevel, FREQUENCIES } from "@/lib/stores/incomeStore";
import { IncomeTypeChips } from "@/components/cards/IncomeTypeChips";
import { NumberInputRupee } from "@/components/shared/NumberInputRupee";
    ...
    return (
        <div className="flex flex-col min-h-screen p-6 pb-24 relative">
            ...
            <div className="relative z-10 flex flex-col min-h-screen">
                ...
                {/* â€”â€”â€” STEP 1: Quick Capture â€”â€”â€” */}
                {step === 1 && (
                    <div className="flex-1 space-y-5">
                        ...
                        <div className="bg-[var(--color-rajya-accent)]/8 border border-[var(--color-rajya-accent)]/20 rounded-xl p-3">
                            <p className="text-xs text-[var(--color-rajya-muted)]">
                                ðŸ’¡ <strong className="text-[var(--color-rajya-text)]">Step 1: Quick Capture</strong> â€” Enter the basics in 60 seconds. Select your income type, name the source, enter the amount, and optionally link your bank account.
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            />
                            {errors.sourceName && <p className="text-[10px] text-[var(--color-rajya-danger)] mt-1">âš  {errors.sourceName}</p>}
                        </div>
                        ...
                        <div>
                            ...
                            />
                            <p className="text-[10px] text-[var(--color-rajya-muted)] mt-1">
                                ðŸ¦ Name the bank account where this income is credited. In Module 5 you can add full bank details and link them.
                            </p>
                        </div>
                        ...
                        {grossIncome > 0 && (
                            <div className="bg-[var(--color-rajya-accent)]/8 border border-[var(--color-rajya-accent)]/20 rounded-xl p-3">
                                ...
                                <p className="text-lg font-bold text-[var(--color-rajya-accent)]">
                                    {monthlyPreview > 0
                                        ? `â‚¹${monthlyPreview.toLocaleString("en-IN")}`
                ...
                {step === "win" && (
                    <div className="flex-1 flex flex-col items-center justify-center space-y-6">
                        ...
                        <div className="space-y-2 text-center">
                            <p className="text-xs text-[var(--color-rajya-muted)]">âœ“ Income source saved</p>
                            <p className="text-xs text-[var(--color-rajya-muted)]">âœ“ Dependency risk updated</p>
                            <p className="text-xs text-[var(--color-rajya-accent)]">â™š {IncomeStore.getRecords().length === 1 ? "King piece placed!" : "Board updated!"}</p>
                        </div>
                ...
                {/* â€”â€”â€” STEP 2: Strength Details â€”â€”â€” */}
                {step === 2 && (
                    <div className="flex-1 space-y-5">
                        {/* Tutorial strip */}
                        <div className="bg-[var(--color-rajya-accent)]/8 border border-[var(--color-rajya-accent)]/20 rounded-xl p-3">
                            <p className="text-xs text-[var(--color-rajya-muted)]">
                                ðŸ’¡ <strong className="text-[var(--color-rajya-text)]">Step 2: Strengthen</strong> â€” Adding risk level and growth % improves your Income Strength Index (ISI). This step is optional â€” you can <em>Skip for Now</em> and add later.
                            </p>
                        </div>
                                ...
                                <div className="bg-white/5 border border-white/10 rounded-xl p-3 mb-3 space-y-2">
                                    ...
                                    <div className="space-y-1.5">
                                        <div className="flex items-start gap-2">
                                            ...
                                            <p className="text-[10px] text-[var(--color-rajya-muted)]">Stable, predictable income â€” e.g. government salary, pension, fixed deposit interest, rent from long-term tenant</p>
                                        </div>
                                        <div className="flex items-start gap-2">
                                            <span className="text-[10px] bg-amber-600/20 text-amber-400 px-1.5 py-0.5 rounded font-bold shrink-0">MEDIUM</span>
                                            <p className="text-[10px] text-[var(--color-rajya-muted)]">Moderately variable â€” e.g. private sector salary, business with regular clients, freelance retainer contracts</p>
                                        </div>
                                        <div className="flex items-start gap-2">
                                            <span className="text-[10px] bg-red-600/20 text-red-400 px-1.5 py-0.5 rounded font-bold shrink-0">HIGH</span>
                                            <p className="text-[10px] text-[var(--color-rajya-muted)]">Unpredictable or volatile â€” e.g. stock market gains, project-based freelancing, crypto income, seasonal business</p>
                                        </div>
                                    </div>
                                    <p className="text-[10px] text-[var(--color-rajya-accent)]">ðŸ’¡ Lower risk = higher Stability score in your ISI analytics</p>
                                </div>
                            )}
