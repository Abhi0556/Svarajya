    ...
    if (!doc) {
        return (
            <div className="flex flex-col min-h-screen p-6 items-center justify-center relative">
                ...
                <button onClick={() => router.push("/pehchaan/records")} className="relative z-10 text-amber-400 text-sm mt-3">Back to Vault</button>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col min-h-screen p-6 pb-24 relative">
            ...
            <div className="relative z-10 flex flex-col min-h-screen">
                ...
                {/* Action buttons */}
                <div className="grid grid-cols-3 gap-2 mb-5">
                    <button onClick={() => router.push(`/pehchaan/records/links/add?docId=${docId}`)} className="bg-white/5 border border-white/10 rounded-xl py-3 flex flex-col items-center gap-1 hover:border-amber-400/30 transition-colors">
                        <Link2 className="w-4 h-4 text-amber-400" />
                        <span className="text-[10px] text-white/40">Add Link</span>
                    </button>
                    <button onClick={() => router.push("/pehchaan/records/renewals")} className="bg-white/5 border border-white/10 rounded-xl py-3 flex flex-col items-center gap-1 hover:border-amber-400/30 transition-colors">
                    ...
                    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-6" onClick={() => setShowDeleteModal(false)}>
                        <div className="bg-slate-900 border border-white/15 rounded-2xl p-5 w-full max-w-sm" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="flex gap-3">
                                ...
                                <button onClick={() => { IdentityStore.deleteDoc(docId); router.push("/pehchaan/records"); }}
